...
public class Slash : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        Vector2 parentPosition = new Vector2(GetComponentInParent<Transform>().position.x, GetComponentInParent<Transform>().position.y);
        Vector2 slashDirection = mousePos - parentPosition;

        float angle = Mathf.Atan2(slashDirection.y, slashDirection.x) * Mathf.Rad2Deg;
        m_rb.rotation = angle;

        transform.localPosition += new Vector3(slashDirection.normalized.x, slashDirection.normalized.y, 0);
